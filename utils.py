...
def load_yolo_weights(model, weights_file):
    ...
    #weights_file="./model_data/mars-small128.pb"
...
def image_preprocess(image, target_size,tempCord1,tempText1, gt_boxes=None):
    ih, iw    = target_size
    h,  w, _  = image.shape
    ...
    #print("\n\t no of keys before loop:",len(tempCord1.keys()))
    if gt_boxes is None:
        return image_paded

    else:

        for indx,ele in enumerate(deepcopy(gt_boxes)):
            #print("\n\t 0.ele:",ele[:4])#,"\t 1:",ele[0],"\t ",ele[1],"\t ",ele[2],"\t ",ele[3])
            #print("\n\t 0.gt_boxes:",gt_boxes[indx,:4])

            tempStr=str("[")+str(ele[0])+", "+str(ele[1])+", "+str(ele[2])+", "+str(ele[3])+"]"
            #print("\n\t str:",tempStr)
            #ele=str(ele[:4])

            #print("\n\t 1.ele:",ele)#,"\t 1:",ele[0],"\t ",ele[1],"\t ",ele[2],"\t ",ele[3])#,"\t word:",tempCord1[str(ele)])
            ...
            txt=tempCord1[str(tempStr)]
            #print("\n\t keys:",tempText1[txt].keys())
            prevEle=ele
            #print("\n\t 1.ele:",ele,"\t txt:",txt,"\tscale:",scale,"\tdw:",dw)
            ele[[0, 2]]=ele[[0, 2]]* scale + dw
            ele[[1, 3]]=ele[[1, 3]]* scale + dh
            tempStr2=str("[")+str(ele[0])+", "+str(ele[1])+", "+str(ele[2])+", "+str(ele[3])+"]"
            tempCord1[tempStr2]=txt
            '''
            print("\n\t 2.ele:",ele) #"\t txt:",txt,"\t new Key Val:",tempCord1[tempStr])
            print("\n\t 22.gt_boxes:",gt_boxes[indx,:4])

            print("\n\t 2.gt_boxes",gt_boxes[indx, [0, 2]] * scale + dw)
            print("\n\t 3.gt_boxes:",gt_boxes[indx, [1, 3]] * scale + dh)
            print("\n\t 4.gt_boxes:",gt_boxes[indx,:4])
            '''
        gt_boxes[:, [0, 2]] = gt_boxes[:, [0, 2]] * scale + dw
        gt_boxes[:, [1, 3]] = gt_boxes[:, [1, 3]] * scale + dh
        #print("\n\t no of keys after loop:", len(tempCord1.keys()))

        return image_paded, gt_boxes,tempCord1
...
def detect_image(Yolo, image_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors=''):
    ...
    cv2.imwrite("./pred1/"+imgName,image)
    return image


def detect_image1(original_image, image_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES,
